...
acton <- acton %>% mutate(
  do.sat=o2.at.sat.base(temp, altitude=283),
    day= as.numeric(is.day(date_time, lat=39.5))
...
acton.LM <- acton %>% # select data frame
  mutate(
    z.mix = z.mix, 
    k.gas = k.gas, 
    irr= as.numeric(is.day(date_time, lat=39.5)),
    wtr=temp ) %>% # add 3 variables 
...
ashort <- acton.LM[-c(1:71, 649:672),]
```

Finally, we use `metab(method="mle")` to estimate GPP, R, and NEP. 
```{r}
## calculate GPP, R, NEP in mg O2 / L / day
out <- metab(acton.LM, method="kalman")
out <- metab(acton.LM, method="mle")
mutate(out,
       GPP.R = GPP+R )
is.day(acton.LM$datetime, lat=39.5)
```
